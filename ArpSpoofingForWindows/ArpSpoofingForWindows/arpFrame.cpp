...
void ARP::setBroadcast(char *srcMac, char *srcIp, char *dstIp)
{
	resetPacket();
	...
	string2Mac(srcMac, srcmac);
	string2Ip(srcIp, srcip);
	string2Ip(dstIp, dstip);
...
void ARP::set4LocalHost(char *localIp)
{
	resetPacket();
	uint8_t srcip[IP_ADDR_LEN];
	uint8_t broadcastMac[ETH_ALEN] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
	string2Ip(localIp, srcip);
	setEthernetDstAddr(broadcastMac);
	setSrcIpAddr(srcip);
	setDstIpAddr(srcip);
}

void ARP::setSpoofing(char *srcMac, char *srcIp, char *dstMac, char *dstIp)
{
	resetPacket();
	mArp->arphdr.op = htons(0x02);

	uint8_t macBuf[ETH_ALEN];
	uint8_t ipBuf[IP_ADDR_LEN];
	string2Mac(srcMac, macBuf);
	setEthernetSrcAddr(macBuf);
	setSrcMacAddr(macBuf);
	string2Mac(dstMac, macBuf);
	setEthernetDstAddr(macBuf);
	setDstMacAddr(macBuf);

	string2Ip(srcIp, ipBuf);
	setSrcIpAddr(ipBuf);
	string2Ip(dstIp, ipBuf);
	setDstIpAddr(ipBuf);
}