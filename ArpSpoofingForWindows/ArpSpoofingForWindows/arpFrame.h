#ifndef _ARP_FRAME_H_
#define _ARP_FRAME_H_
...
public:
	...
	static void mac2String(uint8_t mac[ETH_ALEN], char *str);
	static void string2Mac(char *str, uint8_t mac[ETH_ALEN]);
	static void ip2String(uint8_t ip[IP_ADDR_LEN], char *str);
	static void string2Ip(char *str, uint8_t ip[IP_ADDR_LEN]);
	void printMac();
	void printIp();
	void printPacket();
	...
	void resetPacket();
	void setBroadcast(char *srcMac, char *srcIp, char *dstIp);
	void set4LocalHost(char *localIp);
	void setSpoofing(char *srcMac, char *srcIp, char *dstMac, char *dstIp);
